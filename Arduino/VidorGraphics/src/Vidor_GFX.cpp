...
void Vidor_GFX::fillRect(int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t color, uint8_t alpha) {
  uint32_t rpc[7];
//  if(x<=VIDOR_WIDTH && y<=VIDOR_HEIGHT && x+w<=VIDOR_WIDTH && y+h<=VIDOR_HEIGHT) {
...
void Vidor_GFX::fillRoundRect(int32_t x, int32_t y, uint32_t w, uint32_t h, uint16_t r, uint32_t color, uint8_t alpha) {
  uint32_t rpc[7];
//  if(x<=VIDOR_WIDTH && y<=VIDOR_HEIGHT && x+w<=VIDOR_WIDTH && y+h<=VIDOR_HEIGHT) {
    rpc[0] = RPC_CMD(info.giid, info.chn, 19);
    rpc[1] = x;
    rpc[2] = y;
    rpc[3] = w;
    rpc[4] = h;
    rpc[5] = r;
    rpc[6] = (color & 0xFFFFFF) | ((uint32_t)alpha << 24);
    VidorMailbox.sendCommand(rpc, 7);
//  }
}

void Vidor_GFX::drawRoundRect(int32_t x, int32_t y, uint32_t w, uint32_t h, uint16_t r, uint32_t color, uint8_t alpha) {
  uint32_t rpc[7];
//  if(x<=VIDOR_WIDTH && y<=VIDOR_HEIGHT && x+w<=VIDOR_WIDTH && y+h<=VIDOR_HEIGHT) {
    rpc[0] = RPC_CMD(info.giid, info.chn, 20);
    rpc[1] = x;
    rpc[2] = y;
    rpc[3] = w;
    rpc[4] = h;
...
void Vidor_GFX::runBenchmark(uint32_t flags) {
  uint32_t rpc[2];
    rpc[0] = RPC_CMD(info.giid, info.chn, 21);
    rpc[1] = flags;
